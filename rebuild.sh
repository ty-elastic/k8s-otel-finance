...
docker build --platform $ARCH --build-arg VARIANTS=false --build-arg COURSE=$COURSE --progress plain -t $LOCAL_DOCKER_REPO/$service:$COURSE src/$service
docker push $LOCAL_DOCKER_REPO/$service:$COURSE
